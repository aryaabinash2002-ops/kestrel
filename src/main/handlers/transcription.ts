...
async function testKey(key: SecretKey, value: string): Promise<{ ok: boolean; message: string }> {
  try {
    if (key === 'deepgram') {
      ...
      return res.ok
        ? { ok: true, message: 'Deepgram key accepted' }
        : { ok: false, message: `Deepgram responded ${res.status}` };
    }
    if (key === 'assemblyai') {
      ...
      return res.ok
        ? { ok: true, message: 'AssemblyAI key accepted' }
        : { ok: false, message: `AssemblyAI responded ${res.status}` };
    }
    ...
    return res.ok
      ...
      : { ok: false, message: `Anthropic responded ${res.status}` };
  } catch (err) {
