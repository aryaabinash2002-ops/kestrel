...
export function attachSmokeTest(panel: BrowserWindow, ctx: AppContext): void {
  ...
  if (!out) return;
  if (process.env['KESTREL_SMOKE_SEED_PROFILE'] && !ctx.db.listProfiles().some((p) => p.name === 'Acme — Senior Engineer (demo)')) {
    ctx.db.saveProfile({
      ...
      company: 'Acme',
      type: 'behavioral',
      resumeText: 'Jane Doe — Senior engineer at Globex (2020–2024). Led the billing migration to Stripe with zero downtime, recovering $1.2M annually. Mentored 4 engineers.',
      jdText: 'Acme is hiring a Senior Software Engineer to own the payments platform.',
      stories: [{ id: 'story_demo', title: 'Billing migration', text: 'Migrated billing to Stripe in one quarter with a phased rollout; failures fell from 12% to 1%.' }],
        ...
        if (process.env['KESTREL_SMOKE_SESSION']) {
          ...
          if (process.env['KESTREL_SMOKE_SCREENSHOT']) {
            try {
              ...
              await new Promise((r) => setTimeout(r, 2500));
              summary['screenshots'] = ctx.sessions.session ? ctx.db.listScreenshots(ctx.sessions.session.id) : [];
            } catch (err) {
