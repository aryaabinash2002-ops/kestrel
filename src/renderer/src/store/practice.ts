...
export type PracticePhase = 'idle' | 'starting' | 'speaking' | 'listening' | 'scoring' | 'score' | 'finished';
...
interface PracticeState {
  ...
  start: (opts: { profileId: string | null; setId: string; count: number; useTts: boolean }) => Promise<void>;
...
export const usePractice = create<PracticeState>((set, get) => ({
  ...
  start: async (opts) => {
    set({ phase: 'starting', useTts: opts.useTts, current: null, pending: null, lastScore: null, scores: [], edited: null, error: null, finishedPending: false });
    try {
  ...
  next: () => {
    ...
    if (pending) {
      set({ current: { ...pending, shownAtMs: nowMs() }, pending: null, lastScore: null, edited: null, phase: useTts ? 'speaking' : 'listening' });
  ...
  reset: () => set({ phase: 'idle', current: null, pending: null, lastScore: null, scores: [], edited: null, error: null, finishedPending: false }),
}));
...
on('practice:event', (ev: PracticeEvent) => {
  ...
  switch (ev.type) {
    case 'question': {
      const q: CurrentQuestion = { index: ev.index, total: ev.total, question: ev.question, category: ev.category, shownAtMs: nowMs() };
      if (s.phase === 'score') usePractice.setState({ pending: q });
      else usePractice.setState({ current: q, pending: null, edited: null, lastScore: null, phase: s.useTts ? 'speaking' : 'listening', error: null });
      break;
    }
    case 'listening':
    ...
    case 'scoring':
      ...
      break;
    case 'score':
      usePractice.setState({ phase: 'score', lastScore: ev.score, scores: [...s.scores, ev.score] });
      break;
    case 'finished':
      ...
      break;
    case 'error':
      usePractice.setState({ error: ev.error, phase: s.phase === 'scoring' ? 'listening' : s.phase });
      break;
  }
});
