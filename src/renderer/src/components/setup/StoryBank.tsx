    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <Button
          size="xs"
          variant="outline"
          onClick={() => onChange([...stories, { id: uid('story'), title: '', text: '' }])}
          disabled={stories.length >= 12}
        >
          <Plus /> Add story
        </Button>
      </div>
      <p className="text-[11px] text-muted-foreground">
        Write 5–10 accomplishments once (situation, what you did, the result with a number).
        Behavioral answers pull from these — never from invented experience.
      </p>
      {stories.map((s, i) => (
        <div key={s.id} className="space-y-1.5 rounded-md border border-border/60 p-2">
          <div className="flex items-center gap-1.5">
            ...
            <Input
              value={s.title}
              placeholder="Title, e.g. Billing migration under deadline"
            ...
            <Button
              size="iconSm"
              variant="ghost"
              onClick={() => onChange(stories.filter((x) => x.id !== s.id))}
              title="Remove"
            >
              <Trash2 />
            </Button>
          ...
          <Textarea
            rows={3}
            value={s.text}
            placeholder="Situation → Task → Action → Result (with metrics)"
