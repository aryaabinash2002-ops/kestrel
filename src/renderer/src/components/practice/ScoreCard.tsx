  ...
  return (
    <div className="card-in space-y-3 rounded-lg border border-primary/40 bg-card/80 p-3 shadow-sm">
      <div className="flex items-center gap-3">
        <div className={cn('text-4xl font-bold tabular-nums leading-none', tone(s.score))}>
          {s.score}
        </div>
        ...
        {subs.map((x) => (
          <div key={x.label}>
            ...
            <div className="mt-0.5 h-1.5 overflow-hidden rounded-full bg-secondary">
              <div
                className={cn('h-full rounded-full', bar(x.value))}
                style={{ width: `${x.value * 10}%` }}
              />
            </div>
          </div>
        ))}
      ...
      {s.model_answer && (
        <div>
          <button
            className="no-drag flex items-center gap-1 text-[11px] font-semibold text-muted-foreground hover:text-foreground"
            onClick={() => setShowModel((v) => !v)}
          >
            {showModel ? <ChevronUp className="size-3" /> : <ChevronDown className="size-3" />}{' '}
            Model answer
          </button>
          {showModel && (
            <p className="mt-1 rounded-md bg-muted/40 p-2 text-xs leading-relaxed selectable">
              {s.model_answer}
            </p>
          )}
        </div>
