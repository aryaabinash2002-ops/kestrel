...
export function PracticeHistory({ entries }: { entries: PracticeHistoryEntry[] }) {
  ...
  const overall =
    entries.reduce((a, e) => a + e.avgScore * e.count, 0) /
    Math.max(
      1,
      ...
      <CardHeader>
        <CardTitle className="flex items-center gap-1.5">
          ...
          <span className="ml-auto text-xs font-normal text-muted-foreground">
            {entries.length} session{entries.length === 1 ? '' : 's'} · avg{' '}
            <b className={tone(overall)}>{overall.toFixed(1)}</b>
          </span>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-1.5">
        {entries.map((e) => (
          <div key={e.sessionId} className="rounded-md border border-border/60">
            <button
              className="no-drag flex w-full items-center gap-2 px-2.5 py-1.5 text-left text-xs"
              onClick={() => setOpen(open === e.sessionId ? null : e.sessionId)}
            >
              ...
              <span className={cn('ml-auto font-semibold tabular-nums', tone(e.avgScore))}>
              ...
              <ul className="space-y-1 border-t border-border/60 px-2.5 py-1.5 text-[11px]">
                {e.scores.map((s) => (
                  <li key={s.id} className="flex gap-2">
                    <span
                      className={cn('w-5 shrink-0 font-semibold tabular-nums', tone(s.score.score))}
                    >
                      {s.score.score}
                    </span>
