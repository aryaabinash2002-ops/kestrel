...
export function ExtensionCard() {
  ...
  const badge =
    status === 'capturing' ? (
      <Badge variant="them">Capturing Meet audio</Badge>
    ) : status === 'paired' ? (
      <Badge variant="success">Connected</Badge>
    ) : status === 'listening' ? (
      <Badge variant="secondary">Waiting for extension</Badge>
    ) : (
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Captures only the Meet tab’s audio (no notifications or music) and reads Meet captions as
          a backup. Works in Chrome, Edge, Brave and Arc.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-2 text-xs">
        ...
        <div className="flex gap-2">
          ...
          <Button
            size="sm"
            variant="ghost"
            onClick={() => void invoke('extension:regenerateToken').then(setPairing)}
          >
            <RefreshCw /> New token
          </Button>
        </div>
        {ext?.clientName && (
          <div className="text-muted-foreground">
            Client: {ext.clientName} · {ext.inCall ? 'in a call' : 'not in a call'} · captions{' '}
            {ext.captionsAvailable ? 'available' : 'off'}
          </div>
        )}
        <div className="flex items-center justify-between gap-3 pt-1">
          <div>
            ...
            <div className="text-[11px] text-muted-foreground">
              Starts a session and listening on join; ends it and opens the review when you leave.
            </div>
          </div>
          <Switch
            checked={autoStart}
            onCheckedChange={(v) => void update({ autoStartOnMeetJoin: v })}
          />
        </div>
      </CardContent>
    </Card>
