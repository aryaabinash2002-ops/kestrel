...
export function ChannelStrip({
  ...
  return (
    <div
      className={cn(
        'flex items-center gap-2 rounded-md border border-border/60 bg-card/60 px-2',
        compact ? 'py-1' : 'py-1.5',
      )}
    >
      <Icon
        className={cn(
          'size-3.5 shrink-0',
          tone === 'me' ? 'text-me' : 'text-them',
          !status?.active && 'opacity-40',
        )}
      />
      <span
        className={cn(
          'w-10 shrink-0 text-[10px] font-bold tracking-widest',
          tone === 'me' ? 'text-me' : 'text-them',
        )}
      >
        {isMe ? 'ME' : 'THEM'}
      ...
      <span
        className="ml-1 max-w-[45%] truncate text-[10px] text-muted-foreground"
        title={status?.error ?? status?.deviceLabel ?? ''}
      >
        {status?.error ? (
          ...
          <span className="text-warning">
            {WARNING_LABEL[status.warnings[0] ?? ''] ?? status.warnings[0]}
          </span>
        ) : (
