...
export function ScreenshotCard({
  ...
  return (
    <div
      className={cn(
        'card-in rounded-lg border bg-card/80 shadow-sm',
        streaming ? 'border-primary/50' : 'border-border/70',
        shot.status === 'error' && 'border-destructive/50',
      )}
    >
      <div className="flex items-center gap-1.5 px-3 pt-2">
        <ImageIcon className="size-3.5 text-primary" />
        <span className="text-[11px] text-muted-foreground">
          Screen solution{streaming ? ' — working…' : ''}
        </span>
      ...
      {!collapsed && (
        <div className="px-3 pb-3 pt-1 selectable">
          {shot.result ? (
            <Markdown text={shot.result} streaming={streaming} />
          ) : (
            <p className="text-xs text-muted-foreground caret">Reading the screen…</p>
          )}
        </div>
      )}
    </div>
