...
export function LatencyChart({ samples }: { samples: LatencySample[] }) {
  const pts = samples
    ...
    .reverse()
    .map((s) => ({
      v: (s.firstTokenTs ?? 0) - (s.questionEndTs ?? 0),
      spec: s.speculative,
      restart: s.restarted,
    }));
  if (pts.length === 0) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="h-24 w-full">
      ...
      {pts.map((p, i) => {
        ...
        const fill =
          p.v <= 1000 ? 'var(--success)' : p.v <= 2000 ? 'var(--warning)' : 'var(--destructive)';
        return (
          <g key={i}>
            <rect
              x={i * bw + 1}
              ...
              height={Math.max(1, h)}
              fill={fill}
              opacity={p.spec ? 0.95 : 0.55}
              rx={1}
            />
            {p.restart && (
              <circle cx={i * bw + bw / 2} cy={top - 3} r={1.5} fill="var(--primary)" />
            )}
          </g>
        );
      })}
