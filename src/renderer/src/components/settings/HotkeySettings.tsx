...
const LABELS: Record<HotkeyName, { label: string; hint: string }> = {
  toggleListening: { label: 'Start / stop listening', hint: 'Toggles audio capture and transcription.' },
  answerNow: { label: 'Answer now', hint: 'Answers the last ~30 s of what the other party said.' },
  screenshotSolve: { label: 'Solve what’s on screen', hint: 'Captures a region and sends it to the strong model.' },
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Work in any app, even while Zoom or Meet is focused. Click a binding and press the new combination.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-2">
        {(Object.keys(LABELS) as HotkeyName[]).map((name) => (
          <div key={name} className="flex items-center justify-between gap-3 rounded-md border border-border/60 px-3 py-2">
            <div className="min-w-0">
              <div className="text-sm">{LABELS[name].label}</div>
              {LABELS[name].hint && <div className="text-[11px] text-muted-foreground">{LABELS[name].hint}</div>}
            </div>
            <div className="flex items-center gap-1">
              <button
                className={cn(
                  'no-drag rounded-md border px-2 py-1 font-mono text-xs transition-colors',
                  recording === name ? 'border-primary bg-primary/10 text-primary' : 'border-border hover:bg-accent',
                )}
              ...
              {hotkeys[name] !== DEFAULT_HOTKEYS[name] && (
                <Button size="iconSm" variant="ghost" title="Reset" onClick={() => void update({ hotkeys: { [name]: DEFAULT_HOTKEYS[name] } })}>
                  <RotateCcw />
                </Button>
              )}
            </div>
        ...
        <p className="pt-1 text-[11px] text-muted-foreground">
          Tip: {isMac ? <Kbd>⌘⇧A</Kbd> : <Kbd>Ctrl+Shift+A</Kbd>} answers now, even before a question is auto-detected.
        </p>
      </CardContent>
    </Card>
  );
