...
export function Diagnostics() {
  ...
  const ftl = samples
    .map((s) => (s.firstTokenTs ?? 0) - (s.questionEndTs ?? 0))
    .sort((a, b) => a - b);
  const hl = samples
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Milliseconds from the interviewer’s last word to the first answer token (target ≤ 1000
            p50, ≤ 2000 p95) and to the finished headline (≤ 1500 p50). Negative values mean the
            answer started before they finished.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="grid grid-cols-3 gap-2 text-center">
            ...
            <Stat
              ...
              value={
                samples.length
                  ? Math.round((100 * samples.filter((s) => s.speculative).length) / samples.length)
                  : null
              }
              unit="%"
            />
            <Stat
              label="Restarts"
              value={data ? Math.round(data.speculativeRestartRate * 100) : null}
              unit="%"
            />
          </div>
          ...
          {data?.cache && (
            <p className="text-[11px] text-muted-foreground">
              ...
              — static prefix ≈ {data.cache.prefixTokens.toLocaleString()} tokens, minimum{' '}
              {data.cache.cacheMinimum.toLocaleString()} for the live model.
              {!data.cache.cached &&
                ' A longer résumé/story bank or the Sonnet model enables caching.'}
            </p>
          )}
          <div className="flex gap-2">
            ...
            <Button
              size="sm"
              variant="ghost"
              onClick={() => void invoke('diagnostics:clear').then(refresh)}
            >
              Clear samples
            </Button>
          </div>
            ...
            {data.transcriberStats.map((t) => (
              <div key={t.channel} className="flex justify-between">
                ...
                <span className="text-muted-foreground">
                  {t.interimCount} interim · {t.finalCount} final · {Math.round(t.avgInterimGapMs)}{' '}
                  ms between interims
                </span>
              </div>
            ))}
