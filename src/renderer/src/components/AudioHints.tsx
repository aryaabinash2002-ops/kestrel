...
export function AudioHints({
  ...
  if (
    audio.me.active &&
    them.active &&
    !them.warnings.includes('no-signal') &&
    audio.me.warnings.length === 0 &&
    hints.length === 0
  ) {
    return null;
  }
  if (audio.me.warnings.includes('bluetooth-headset')) {
    hints.push({
      ...
      text: 'Bluetooth headset is in low-quality call mode — use the laptop mic or a wired headset for better transcripts.',
    });
  }
  ...
  return (
    <div className="mx-3 mt-2 space-y-1">
      {hints.map((h, i) => (
        <div
          key={i}
          className="flex items-start gap-2 rounded-md border border-warning/40 bg-warning/10 px-2 py-1 text-[11px] text-warning"
        >
