...
export function TranscriptionStatus({
  ...
  states: Record<'ME' | 'THEM', TranscriptionState | null>;
  listening: boolean;
}) {
  const items = [states.ME, states.THEM].filter((s): s is TranscriptionState => !!s);
  const bad = items.filter(
    (s) => s.status === 'reconnecting' || s.status === 'error' || s.status === 'connecting',
  );
  ...
  return (
    <div
      className={cn(
        'mx-3 mt-2 flex items-center gap-2 rounded-md border px-2 py-1 text-[11px]',
        err
