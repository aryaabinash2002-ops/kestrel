...
export function defaultResponder(body) {
  ...
  const content = typeof user?.content === 'string' ? user.content : (user?.content ?? []).map((b) => b.text ?? '').join('\n');
  ...
  if (fmt && fmt.type === 'json_schema') {
    ...
    if ('is_question' in props) {
      ...
      const type = /time you|conflict|challenge|strength|weakness|tell me about/i.test(q) ? 'behavioral' : /how are you|good morning|hear me/i.test(q) ? 'smalltalk' : 'technical';
      return JSON.stringify({ is_question: isQ, question: q.replace(/\s+/g, ' '), type });
    }
    if ('score' in props) {
      return JSON.stringify({ score: 7, relevance: 8, structure: 6, specificity: 7, conciseness: 7, strengths: ['Clear outcome'], improve_one_thing: 'Quantify the result.', model_answer: 'At Globex I led the billing migration, cutting invoice errors by 40%.' });
    }
    if ('summary' in props) {
      return JSON.stringify({ summary: 'A solid conversation.', questions: [q], weakSpots: ['Answer was vague on metrics — add numbers next time.'], followUpEmail: 'Hi [Name],\n\nThank you for the conversation today.', actionItems: ['Send follow-up email'] });
    }
    return JSON.stringify({});
  }
  if (/Follow-up request from the user/.test(content)) return `Here is a shorter version.\nKeep it to one example and one number.`;
  if (/screenshot|## Problem/i.test(body.system?.[0]?.text ?? body.system ?? '')) return '## Problem\nTwo-sum.\n\n## Approach\n- Hash map.\n\n## Pseudocode\nloop\n\n## Complexity\nO(n) time, O(n) space\n\n## Edge cases\n- empty\n\n## Code\n```python\ndef two_sum(a, t):\n    seen = {}\n```';
...
export function startFakeAnthropic(opts = {}) {
  ...
  const server = createServer((req, res) => {
    ...
    req.on('end', async () => {
      ...
      const usage = { input_tokens: 900, output_tokens: 0, cache_read_input_tokens: 0, cache_creation_input_tokens: 0 };
      const sysBlocks = Array.isArray(body.system) ? body.system : [];
      if (sysBlocks.some((b) => b.cache_control)) {
        const seen = requests.filter((r) => r !== rec && JSON.stringify(r.body.system) === JSON.stringify(body.system)).length;
        if (seen) usage.cache_read_input_tokens = 800;
        else usage.cache_creation_input_tokens = 800;
      }
      if (body.max_tokens === 1) {
        res.writeHead(200, { 'content-type': 'application/json' });
        res.end(JSON.stringify({ id: 'msg_warm', type: 'message', role: 'assistant', model: body.model, content: [{ type: 'text', text: 'ok' }], stop_reason: 'max_tokens', stop_sequence: null, usage: { ...usage, output_tokens: 1 } }));
        return;
      }
      if (!body.stream) {
        res.writeHead(200, { 'content-type': 'application/json' });
        res.end(JSON.stringify({ id: 'msg_1', type: 'message', role: 'assistant', model: body.model, content: [{ type: 'text', text }], stop_reason: 'end_turn', stop_sequence: null, usage: { ...usage, output_tokens: 50 } }));
        return;
      }
      res.writeHead(200, { 'content-type': 'text/event-stream', 'cache-control': 'no-cache', connection: 'keep-alive' });
      const send = (event, data) => res.write(`event: ${event}\ndata: ${JSON.stringify(data)}\n\n`);
      send('message_start', { type: 'message_start', message: { id: 'msg_1', type: 'message', role: 'assistant', model: body.model, content: [], stop_reason: null, stop_sequence: null, usage } });
