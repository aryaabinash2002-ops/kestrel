...
describe('AnswerEngine', () => {
  it('streams an answer: start → headline → deltas → done, with latency recorded', async () => {
    ...
    expect(done.card.latency.headlineDoneTs).toBeGreaterThanOrEqual(
      done.card.latency.firstTokenTs!,
    );
    ...
    expect(JSON.stringify(req.body.messages)).toContain(
      'Question: Tell me about a time you led a migration?',
    );
  });
  ...
  it('ignores a re-detection of the same question while it streams', async () => {
    ...
    await until(() => events.some((e) => e.type === 'start'));
    engine.requestAuto({
      question: 'Walk me through your resume please',
      kind: 'auto',
      questionEndTs: Date.now(),
    });
    await until(() => events.some((e) => e.type === 'done'));
  ...
  it('queues a new question when the current answer is ≥70 % done', async () => {
    ...
    engine.requestAuto({
      question: 'Completely different second question?',
      kind: 'auto',
      questionEndTs: Date.now(),
    });
    ...
    expect(starts.map((s) => s.card.question)).toEqual([
      'First question?',
  ...
  it('reports an error card on authentication failure', async () => {
    ...
    const engine = new AnswerEngine({
      llm,
      sessions: sessions as never,
      db: { saveAnswer() {}, saveLatency() {} } as never,
      getSettings: () => DEFAULT_SETTINGS,
    });
    const events: AnswerEvent[] = [];
    ...
    const err = events.find(
      (e): e is Extract<AnswerEvent, { type: 'error' }> => e.type === 'error',
    )!;
