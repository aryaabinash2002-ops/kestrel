...
import { buildLivePrompt, cacheMinimumTokens, formatTranscript, profileToContext } from '@main/llm/ContextBuilder';
...
const profile: Profile = {
  ...
  stories: [{ id: 's1', title: 'Billing migration', text: 'Migrated billing to Stripe in 4 months with zero downtime.' }],
...
function utt(speaker: 'ME' | 'THEM', text: string, endMs: number): Utterance {
  return { id: `u${endMs}`, sessionId: 's', speaker, text, startMs: endMs - 1000, endMs, isFinal: true, source: 'stt' };
}

describe('prompt rendering', () => {
  it('fills variables and conditional blocks', () => {
    const out = renderTemplate('Hi {name}. {if flag}ON{/if}{if other}OFF{/if} {missing}!', { name: 'Jane', flag: true, other: false });
    expect(out).toBe('Hi Jane. ON !');
  });

  it('renders the live-answer system prompt with résumé, JD, stories and STAR rule for behavioral', () => {
    const ctx = profileToContext(profile, { length: 'short', tone: 'confident', language: 'English' });
    const built = buildLivePrompt({ template: DEFAULT_PROMPTS.live_answer, profile: ctx, model: 'claude-sonnet-5', utterances: [], nowMs: 0, summary: null, question: 'Tell me about a challenge' });
...
describe('ContextBuilder trimming + caching', () => {
  it('keeps only the last 5 minutes of transcript, newest-first budget', () => {
    ...
    const utts = [utt('THEM', 'old question', nowMs - 6 * 60 * 1000), utt('ME', 'recent answer', nowMs - 60 * 1000), utt('THEM', 'newest', nowMs - 1000)];
  ...
  it('caps the transcript by characters keeping the newest lines', () => {
    const nowMs = 100000;
    const utts = Array.from({ length: 200 }, (_, i) => utt('THEM', `line ${i} ` + 'x'.repeat(80), 1000 + i * 100));
  ...
  it('only marks cache_control when the prefix reaches the model minimum', () => {
    const ctx = profileToContext(profile, { length: 'short', tone: 'confident', language: 'English' });
    const small = buildLivePrompt({ template: DEFAULT_PROMPTS.live_answer, profile: ctx, model: 'claude-haiku-4-5-20251001', utterances: [], nowMs: 0, summary: null, question: 'q' });
    ...
    const big = buildLivePrompt({ template: DEFAULT_PROMPTS.live_answer, profile: bigCtx, model: 'claude-haiku-4-5-20251001', utterances: [], nowMs: 0, summary: null, question: 'q' });
  ...
  it('includes the running summary before the recent transcript', () => {
    ...
    const built = buildLivePrompt({ template: DEFAULT_PROMPTS.live_answer, profile: ctx, model: 'claude-sonnet-5', utterances: [utt('THEM', 'hi', 500)], nowMs: 1000, summary: 'Earlier they discussed salary.', question: 'q' });
    expect(built.user.indexOf('<earlier_summary>')).toBeLessThan(built.user.indexOf('<transcript_recent>'));
