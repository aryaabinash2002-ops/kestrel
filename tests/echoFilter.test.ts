...
describe('EchoFilter', () => {
  it('drops ME text that duplicates recent THEM speech', () => {
    ...
    expect(f.isEcho('Tell me about a time you had a conflict with a co-worker.', t + 900)).toBe(true);
  });
  ...
  it('keeps genuinely different ME speech', () => {
    ...
    expect(f.isEcho('Sure. Last year at Acme I disagreed with our lead about the rollout plan', t + 500)).toBe(false);
  ...
  it('similarity helpers behave', () => {
    ...
    expect(textSimilarity('walk me through your resume', 'Can you walk me through your resume please?')).toBeGreaterThan(0.7);
  });
});
